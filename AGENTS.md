...
- PC 商城：代码中保留路由、开关和部署槽位，但当前仓库没有 PC 商城源码，也没有 `server/public/pc/` 构建产物。
...
- `/pc/...`：尝试返回 `server/public/pc/index.html`；当前仓库缺少该文件。
...
- PC 商城前台只保留接入点：`route/route.php` 的 `/pc/:any`、`application/index/controller/Index.php` 的桌面端模板选择、admin 中的 PC 设置，以及配置表开关。
- 当前仓库没有 `server/public/pc/`，也没有 Nuxt/Vue PC 商城源码。README 也说明免费企业版不包含 PC 商城端。

因此，收到 PC 商城页面需求时先确认源码是否由用户另行提供。没有源码时，不要假装存在 PC 工程，不要把 admin 当作 PC 商城，也不要直接在缺失的编译目录中手写一套页面。引入或新建 PC 商城属于独立架构决策，需先确定技术栈、API 复用方式、登录/支付/SEO 和部署目录。
...
5. **P4 PC 商城决策**：确认商业版源码引入、现有源码迁入或新建 PC 前台三选一，再制定 SEO、账号、支付和部署方案。
