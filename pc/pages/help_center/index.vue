  ...
  <div class="help-center-container">
    ...
    <div class="help-center-box">
      ...
      <div class="article-lists-container ml16 bg-white">
        <div  v-show="!dataNull">
          <div>
            <nuxt-link :to="'/help_center/help_center_detail?id=' + item.id" class="article-item row-between bg-white" v-for="(item) in articleList" :key="item.id">
              <div>
...
export default {
  ...
  async asyncData({$get, $post}) {
    let dataNull = true;
    const banner = $get("ad_content/lists", { params: { pid: 11, client: 2 } });
    const { data: bannerList } = await banner;
    let res = await $get('help/category');
    if(res.code == 1) {
      let categoryList = res.data;
      let currentId = 0
      let listsRes = await $get("help/lists", {params: {id: currentId, page_size: 10}});
      if(listsRes.code == 1) {
  ...
  methods: {
    async changePage(current) {
      let res = await this.$get("help/lists", {params: {id: this.currentId, page_no: current, page_size: 10}});
      if(res.code == 1) {
