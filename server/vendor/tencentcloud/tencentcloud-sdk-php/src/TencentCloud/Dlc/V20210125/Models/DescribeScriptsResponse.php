 ...
 *
 * @method array getScripts() 获取Script列表
注意：此字段可能返回 null，表示取不到有效值。
 * @method void setScripts(array $Scripts) 设置Script列表
注意：此字段可能返回 null，表示取不到有效值。
...
注意：此字段可能返回 null，表示取不到有效值。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("Scripts",$param) and $param["Scripts"] !== null) {
            $this->Scripts = [];
            foreach ($param["Scripts"] as $key => $value){
                $obj = new Script();
                $obj->deserialize($value);
                array_push($this->Scripts, $obj);
            }
        }
