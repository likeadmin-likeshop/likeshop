 ...
 *
 * @method array getTableList() 获取数据表对象列表。
 * @method void setTableList(array $TableList) 设置数据表对象列表。
...
class DescribeTablesResponse extends AbstractModel
{
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("TableList",$param) and $param["TableList"] !== null) {
            $this->TableList = [];
            foreach ($param["TableList"] as $key => $value){
                $obj = new TableResponseInfo();
                $obj->deserialize($value);
                array_push($this->TableList, $obj);
            }
        }
