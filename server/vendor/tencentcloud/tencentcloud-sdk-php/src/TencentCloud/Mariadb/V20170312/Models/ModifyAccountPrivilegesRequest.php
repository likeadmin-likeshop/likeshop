 ...
 *
 * @method string getInstanceId() 获取实例 ID，格式如：tdsql-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同。
 * @method void setInstanceId(string $InstanceId) 设置实例 ID，格式如：tdsql-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同。
...
{
    /**
     * @var string 实例 ID，格式如：tdsql-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同。
     */
    public $InstanceId;
...
注意，不传该参数表示保留现有权限，如需清除，请在复杂类型Privileges字段传空数组。
    ...
    /**
     * @param string $InstanceId 实例 ID，格式如：tdsql-c1nl9rpv，与云数据库控制台页面中显示的实例 ID 相同。
...
注意，不传该参数表示保留现有权限，如需清除，请在复杂类型Privileges字段传空数组。
    ...
    public function deserialize($param)
    {
        ...
        if (array_key_exists("FunctionPrivileges",$param) and $param["FunctionPrivileges"] !== null) {
            $this->FunctionPrivileges = [];
            foreach ($param["FunctionPrivileges"] as $key => $value){
                $obj = new FunctionPrivilege();
                $obj->deserialize($value);
                array_push($this->FunctionPrivileges, $obj);
            }
        }

        if (array_key_exists("ProcedurePrivileges",$param) and $param["ProcedurePrivileges"] !== null) {
            $this->ProcedurePrivileges = [];
            foreach ($param["ProcedurePrivileges"] as $key => $value){
                $obj = new ProcedurePrivilege();
                $obj->deserialize($value);
                array_push($this->ProcedurePrivileges, $obj);
            }
        }
    }
}
