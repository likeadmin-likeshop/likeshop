 ...
 *
 * @method string getSourceQueueName() 获取死信策略源队列名称，调用本接口会清空该队列的死信队列策略。
 * @method void setSourceQueueName(string $SourceQueueName) 设置死信策略源队列名称，调用本接口会清空该队列的死信队列策略。
 */
class UnbindCmqDeadLetterRequest extends AbstractModel
{
    ...
    /**
     * @param string $SourceQueueName 死信策略源队列名称，调用本接口会清空该队列的死信队列策略。
     */
