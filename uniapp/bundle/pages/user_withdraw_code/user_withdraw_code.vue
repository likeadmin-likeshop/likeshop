        ...
        <view class="withdraw-code-container mt10">
            <view class="withdraw-code-contain">
                <navigator
                    ...
                    :key="index"
                    class="withdraw-code-item bg-white"
                    hover-class="none"
                ...
                >
                    ...
                    <view class="row-between mt10">
                        ...
                        <view
                            class="withdraw-status xs"
                            :class="{
                                'error-status': item.status == 2 || item.status == 4,
                                'common-status': item.status == 3
                            }"
                            >{{ item.status_text }}</view
                        >
                    </view>
                    <view
                        v-if="item.description && item.status == 4"
                        class="primary mt10 line1 xs"
...
export default {
    ...
    methods: {
        getWithdrawRecordsFun() {
            let { loadingStatus, withdrawRecords, page } = this
